...
// The backend sends one combined string: `... was rejected: <reason>`. Split it so
// the reason can be shown on its own line as a distinct block. Falls back to the
// whole message (no separate reason) if the expected marker is absent.
  ...
  useEffect(() => {
    ...
    const refresh = async () => {
      ...
      try {
        ...
        if (active) setError("Could not load rejection notifications.");
      } finally {
        pending = false;
      }
    };
    ...
    >
      <header className="flex items-center justify-between gap-3 border-b border-gray-100 px-4 py-3 dark:border-gray-700">
        <div className="flex items-center gap-3">
          <span
            aria-hidden="true"
            className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-danger-100 text-danger-600 dark:bg-danger-900/40 dark:text-danger-300"
          >
      ...
      {visible.length === 0 ? (
        <p className="px-4 py-6 text-center text-sm text-gray-500 dark:text-gray-400">
          No unread rejections. Choose “Show all” to review past decisions.
        </p>
      ) : (
            ...
            >
              <span
                ...
                className={`mt-1.5 h-2 w-2 shrink-0 rounded-full ${
                  n.read ? "bg-transparent" : "bg-danger-500"
                }`}
              />
              <div className="min-w-0 flex-1">
                ...
                <div className="mt-1 flex flex-wrap items-center gap-2 text-xs text-gray-400 dark:text-gray-500">
                  ...
                  {!n.read && (
                    <span className="rounded-full bg-danger-100 px-2 py-0.5 font-medium text-danger-700 dark:bg-danger-900/40 dark:text-danger-300">
                      New
                    </span>
                  )}
                </div>

...
`docker-compose` defines the shared three-tier Docker Compose integration environment: frontend, backend, and PostgreSQL. The backend is published directly on `localhost:8080`; no gateway or storage/messaging emulators are part of this stack.

The Compose build contexts expect `../frontend` for the React/Vite frontend, `../backend` for the NestJS backend service, and `../database/postgresql` for the local PostgreSQL image in this checkout. PostgreSQL initialization assets are built into that image from `database/postgresql/init`.

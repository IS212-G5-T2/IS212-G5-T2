...
The Compose build contexts expect `../frontend` for the React/Vite frontend, `../backend` for the NestJS backend service, and `../database/postgresql` for the local PostgreSQL image. Database assets are owned separately from the Compose orchestration.
...
volumes can be reseeded without a reset:

```sh
docker compose exec -T postgres psql -U spm -d spm -v ON_ERROR_STOP=1 -f - < ../database/postgresql/init/002_seed_data.sql
```

Use the documented development accounts and password in
`database/README.md`; never reuse them outside local development.
...
| Path | Responsibility |
| --- | --- |
| `frontend` | React/Vite frontend source and frontend container image |
| `backend` | NestJS backend source and backend container image |
| `database/postgresql` | Local PostgreSQL image and initialization assets |
| `docker-compose` | Three-tier Docker Compose orchestration and local stack docs |
...
The Compose configuration targets `backend`. To integrate a different service:

1. Point the Compose build context to the intended service's root-level directory.
...
PostgreSQL init scripts are built into the local image from `database/postgresql/init`. These scripts run only when Docker creates a fresh `postgres-data` volume. For already-initialized local databases, apply changes manually or explicitly reset data with `docker compose down -v` from this directory when it is safe to discard local state.
...
The local `spm` PostgreSQL database stores requests in `events`. `001_schema.sql` defines every local table; `002_seed_data.sql` adds the local RBAC, accounts, health check, and fictional Submitted event. Records persist in Docker's `postgres-data` volume. Dates/times use `timestamptz`; the API/browser handles local-time display. These files are for fresh volumes; use backend migrations for existing-volume schema changes.
...
(see `backend/AGENTS.md`'s events-boundary policy). After backend

...
## 2026-09-13 - Claude (Opus 4.8) - Draft save confirmation/error pop-ups and commit SPM-37

- Issue: https://is212-g5-t2.atlassian.net/browse/SPM-37 (AC6 UX refinement plus repository handoff).
- Areas: frontend draft form (`DraftRequestForm`), `DraftRequestPage`/`MyRequestsPage` saved-state wiring, draft form/page Vitest suites.
- Summary: Reworked AC6 feedback into modal pop-ups. A successful save now shows a "Draft saved" dialog whose OK button redirects to My Requests (navigation deferred until acknowledged); a failed save shows an error dialog that retains entered values and offers retry, with recognised statuses keeping their specific message, network failures keeping their actionable message, and any other status (e.g. 500/503) or non-standard failure falling back to a generic "Something went wrong while saving your draft. Please try again." Removed the on-mount success banner (reopening a draft no longer pops up) and the redundant My Requests saved banner. Renamed test cases with their AC numbers, added a parametrised failure-case suite, and removed the requirement-grid test per user request.
- Checks: Frontend Vitest 19/19 and TypeScript compilation passed using existing installed tooling. Backend suites and Playwright were not run this session. No clean dependency install performed.
- Handoff: Committed all previously-uncommitted SPM-37 work as `feat(SPM-37): save event request as a draft` (commit 69c5986, 49 files) and pushed feature/SPM-37-save-event-request-as-a-draft to origin. PR into dev is still open work: the local gh CLI is authenticated as a bot without repo access and the GitHub MCP failed to connect, so the pull request must be opened from the user's account (compare dev...feature/SPM-37-save-event-request-as-a-draft).
- Acceptance: AC7 organisation isolation and AC8 end-to-end (post-submit lockout via the real create/submit flow) remain deferred to later sprints, consistent with prior entries; no claim of full SPM-37 integration.

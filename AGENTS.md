...
`dev` is the latest shared branch and the default base for all new work. When starting new implementation, documentation, test, chore, or refactor work, create a focused branch from the latest `dev` and open the pull request back into `dev`.
...
| Urgent fix | `fix/<ticket_id>-<ticket_name>` or `hotfix/<ticket_id>-<ticket_name>` from `dev` | `dev` | Repair the latest shared branch without a separate production branch. |

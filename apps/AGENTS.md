...
- Front-facing application work belongs here, including new deployable frontend experiences or client-facing interfaces.
- Applications in this area are not deployed through Kubernetes. Use the app's documented non-Kubernetes hosting path and coordinate only the API/backend boundary with `services/` and `platform/`.
...
- Backend microservice work belongs under `services/`; deployment strategy and shared infrastructure configuration belong under `platform/`. Identify cross-area changes explicitly and apply that area's guidance before editing across the application boundary.
